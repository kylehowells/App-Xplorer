...
// MARK: - Iroh Transport
	...
	class IrohTransport: Transport {
		...
		func connect() async throws {
			...
			// Build peer address
			let peerPubkey = try PublicKey.fromString(s: self.peerNodeId)
			let peerAddr = NodeAddr(nodeId: peerPubkey, derpUrl: peerRelayUrl, addresses: peerDirectAddresses)

			fputs("Peer relay URL: \(self.peerRelayUrl ?? "none")\n", stderr)
			fputs("Peer direct addresses: \(self.peerDirectAddresses)\n", stderr)
			...
			fputs("Connecting to peer \(self.peerNodeId.prefix(16))...\n", stderr)
...
struct ParsedArgs {
	...
	var isIroh: Bool {
		return self.target.hasPrefix("iroh:")
	}

	/// Get the Iroh node ID (without prefix)
	var irohNodeId: String? {
		guard self.isIroh else { return nil }

		return String(self.target.dropFirst(5)) // Remove "iroh:" prefix
	}
}
