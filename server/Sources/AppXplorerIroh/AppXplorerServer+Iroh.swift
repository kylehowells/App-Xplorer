...
extension AppXplorerServer {
	...
	/// try server.start()
	/// ```
	public static func withIroh(storagePath: String? = nil) async throws -> (AppXplorerServer, IrohTransportAdapter) {
	...
	/// try server.start()
	/// ```
	public static func withHTTPAndIroh(
